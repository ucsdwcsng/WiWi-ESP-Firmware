...
unsigned int maxPullRangeSetting = RNG_50_PPM;
int maxPullRange = 50; // PPM
float center_freq = 40; // Mhz

void set_max_pull_range(unsigned int pull_range){
  Wire.begin(DCTCXO_SDA, DCTCXO_SCL);

  Wire.beginTransmission(DCTCXO_ADDR);
  Wire.write(0x2); // register address
  Wire.write(0x0); // unused MSB
  Wire.write(pull_range & 15); // LS 4 bits
  int i2c_status = Wire.endTransmission();
  sprintf(print_buffer, "Pull Range setting Status : 0x%x\n", i2c_status);
...
void apply_freq_change(float del_freq_hz){
  ...
  // TODO: this would be dependendent on current estimate of center freq?
  float del_freq = del_freq_hz / center_freq;  // in PPM

  if (del_freq > 0) {
    toRet = (int) round(del_freq / maxPullRange * (1 << 25 - 1));
  } else {
    toRet = (int) round(del_freq / maxPullRange * (1 << 25));
  }

  sprintf(print_buffer, "Writing : 0x%x, %d, %f Hz\n", toRet, toRet, del_freq_hz);
  Serial.write(print_buffer);

  Wire.begin(DCTCXO_SDA, DCTCXO_SCL);

  Wire.beginTransmission(DCTCXO_ADDR);
  Wire.write(0x0); // register address

  // Follow fig 37 in SIT5501 datasheet
  Wire.write((toRet >> 8) & 255); // bits [8,15], 65280
  Wire.write(toRet & 255); // bits [0, 7]
  Wire.write((0x1 << 2) | ((toRet >> 24) & 3)); // OE, bits[24, 25]
  Wire.write((toRet >> 16) & 255); // bits [16, 23], 16711680
  int i2c_status = Wire.endTransmission();
...
void espmgmt_setup()
{
  ...
  set_max_pull_range(maxPullRangeSetting);
...
void expmgmt_loop() {
  ...
  if ( (loop_counter % 10) == 0) {
    // do_wifi_scan();   
    // do_wifi_connect_test();
    int step_hz = maxPullRange*4;
    float freq_change = ((((int) (loop_counter / 10)) % 10) * step_hz) - (maxPullRange*40/2); // assuming 200 PPM max pull range, 40 MHz clock
    apply_freq_change(freq_change);
    // sprintf(print_buffer, "Loop counter: %d, freq change: %f\n", loop_counter, freq_change);
    // Serial.write(print_buffer);
  }
